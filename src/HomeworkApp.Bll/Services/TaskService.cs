...
public class TaskService : ITaskService
{
    ...
    {
        string cacheKey = $"cache_task_messages:{taskId}";
        var cachedTaskMessagesJson = await _distributedCache.GetStringAsync(cacheKey, token);
        if (!string.IsNullOrEmpty(cachedTaskMessagesJson))
        {
            var cachedTaskMessages = JsonSerializer.Deserialize<TaskMessage[]>(cachedTaskMessagesJson);
            if (cachedTaskMessages is not null)
            {
                return cachedTaskMessages;
            }
        }

        var taskComments = await _taskCommentRepository.Get(
        ...
        var taskMessages = taskComments.Select(taskComment =>
            new TaskMessage()
            {
        ...
        var taskMessagesJson = JsonSerializer.Serialize(taskMessages.Take(5));
        await _distributedCache.SetStringAsync(
            cacheKey, 
            taskMessagesJson,
            new DistributedCacheEntryOptions()
